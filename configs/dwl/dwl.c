...
struct Monitor {
	...
	const Layout *lt[2];
	int gappx;           /* horizontal gap between windows */
...
createmon(struct wl_listener *listener, void *data)
{
    ...
    struct wlr_output_mode *prefered, *mode;
	const MonitorRule *r;
	size_t i;
    ...
    prefered = wlr_output_preferred_mode(wlr_output);
    wl_list_for_each(mode, &wlr_output->modes, link) {
        printf(" >%dx%d @ %d\n", prefered->width, prefered->height, prefered->refresh);
        if (mode->refresh > prefered->refresh && 
                (mode->width >= prefered->width || mode->height >= prefered->height)) {
            prefered = mode;
        }
    }
    printf("prefered %dx%d @ %d\n", prefered->width, prefered->height, prefered->refresh);
	wlr_output_state_set_mode(&state, prefered);
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster ? (int)roundf((m->w.width + m->gappx*ie) * m->mfact) : 0;
	else
		mw = m->w.width - 2*m->gappx*oe + m->gappx*ie;
	i = 0;
	my = ty = m->gappx*oe;
	wl_list_for_each(c, &clients, link) {
		if (!VISIBLEON(c, m) || c->isfloating || c->isfullscreen)
		...
		if (i < m->nmaster) {
			r = MIN(n, m->nmaster) - i;
			h = (m->w.height - my - m->gappx*oe - m->gappx*ie * (r - 1)) / r;
			resize(c, (struct wlr_box){.x = m->w.x + m->gappx*oe, .y = m->w.y + my,
				.width = mw - m->gappx*ie, .height = h}, 0);
			my += c->geom.height + m->gappx*ie;
		} else {
			r = n - i;
			h = (m->w.height - ty - m->gappx*oe - m->gappx*ie * (r - 1)) / r;
			resize(c, (struct wlr_box){.x = m->w.x + mw + m->gappx*oe, .y = m->w.y + ty,
				.width = m->w.width - mw - 2*m->gappx*oe, .height = h}, 0);
			ty += c->geom.height + m->gappx*ie;
		}
		i++;
	}
}
